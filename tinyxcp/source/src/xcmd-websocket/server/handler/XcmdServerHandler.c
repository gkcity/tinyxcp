...
static void _sendError(ChannelHandler *thiz, Channel *channel, const char *id, int32_t status, const char *description)
{
    ...
    XcpMessage * message = IQError_New(XcmdServerHandlerContext_NextId(context), status, description);
    if (message == NULL)
    {
...
static bool _MessageRead(ChannelHandler *thiz, Channel *channel, ChannelDataType type, const void *data, uint32_t len)
{
    ...
    switch (message->iq.type)
    {
        ...
        case IQ_TYPE_RESULT:
        case IQ_TYPE_ERROR:
            XcmdServerHandlerContext_Handle(context, message);
            break;
...
TINY_LOR
static bool _HandleWebsocketHandshake(ChannelHandler *thiz, Channel *channel, XcmdServerHandlerContext *context, HttpMessage *request)
{
    bool ret = true;
    ...
    do
    {
        ...
        SocketChannel_AddBefore(channel, XcmdServerHandler_Name, WebSocketFrameCodec());
        SocketChannel_AddBefore(channel, XcmdServerHandler_Name, MessageCodec(context->data->device, MESSAGE_CODEC_SERVER));
    } while (false);
...
TINY_LOR
static bool _HandleHttpRequest(Channel *channel, XcmdServerHandlerContext *context, HttpMessage *request)
{
    bool ret = false;
...
static void _Ping(ChannelHandler *thiz, Channel *channel)
{
    ...
    do
    {
        XcpMessage * message = Ping_New(XcmdServerHandlerContext_NextId(context));
        if (message == NULL)
        {
        ...
        if (RET_FAILED(XcmdServerHandlerContext_AddHandler(thiz->context, message->iq.id, _handlePong, thiz)))
        {
            LOG_D(TAG, "XcmdServerHandlerContext_AddHandler FAILED!");
            return;
        }
...
TINY_LOR
static void XcmdServerHandler_Dispose(ChannelHandler *thiz)
{
    RETURN_IF_FAIL(thiz);

    XcmdServerHandlerContext_Delete((XcmdServerHandlerContext *) (thiz->context));
    memset(thiz, 0, sizeof(ChannelHandler));
}

TINY_LOR
static void XcmdServerHandler_Delete(ChannelHandler *thiz)
{
    XcmdServerHandler_Dispose(thiz);
    tiny_free(thiz);
}

TINY_LOR
static TinyRet XcmdServerHandler_Construct(ChannelHandler *thiz, XcmdServerContext *data)
{
    ...
    do
    {
        ...
        strncpy(thiz->name, XcmdServerHandler_Name, CHANNEL_HANDLER_NAME_LEN);

        thiz->invalid = false;
        thiz->onRemove = XcmdServerHandler_Delete;
        ...
        thiz->context = XcmdServerHandlerContext_New(data);
        if (thiz->context == NULL)
        {
            LOG_E(TAG, "XcmdServerHandlerContext_New FAILED");
            ret = TINY_RET_E_NEW;
            break;
        }
    } while (0);
...
ChannelHandler * XcmdServerHandler(void)
{
    ...
    do
    {
        ...
        if (RET_FAILED(XcmdServerHandler_Construct(thiz, data)))
        {
            XcmdServerHandler_Delete(thiz);
            thiz = NULL;
            break;
        }
    } while (0);
