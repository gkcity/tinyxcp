...
{
    XcpClientHandlerContext *context = (XcpClientHandlerContext *) (thiz->context);
...
static void _sendError(ChannelHandler *thiz, Channel *channel, const char *id, int32_t status, const char *description)
{
    ...
    XcpMessage * message = IQError_New(XcpClientHandlerContext_NextId(context), status, description);
    if (message == NULL)
    {
...
{
    XcpClientHandlerContext *context = (XcpClientHandlerContext *) (ctx);

    LOG_D(TAG, "onVerifySuccess");

    SocketChannel_AddBefore(context->channel, MessageCodec_Name, XcpBinaryCodec(serverToDeviceKey, deviceToServerKey));
}
...
static bool _ChannelRead(ChannelHandler *thiz, Channel *channel, ChannelDataType type, const void *data, uint32_t len)
{
    ...
    switch (message->iq.type)
    {
        ...
        case IQ_TYPE_RESULT:
        case IQ_TYPE_ERROR:
            XcpClientHandlerContext_Handle(context, message);
            break;
...
static void _Ping(ChannelHandler *thiz, Channel *channel)
{
    ...
    do
    {
        XcpMessage * message = Ping_New(XcpClientHandlerContext_NextId(context));
        if (message == NULL)
        {
        ...
        if (RET_FAILED(XcpClientHandlerContext_AddHandler(thiz->context, message->iq.id, _handlePong, thiz)))
        {
...
TINY_LOR
static void XcpClientHandler_Dispose(ChannelHandler *thiz)
{
    RETURN_IF_FAIL(thiz);

    XcpClientHandlerContext_Delete((XcpClientHandlerContext *) (thiz->context));
    memset(thiz, 0, sizeof(ChannelHandler));
}

TINY_LOR
static void XcpClientHandler_Delete(ChannelHandler *thiz)
{
    XcpClientHandler_Dispose(thiz);
    tiny_free(thiz);
}

TINY_LOR
static TinyRet XcpClientHandler_Construct(ChannelHandler *thiz, Device *device)
{
    ...
    do
    {
        ...
        strncpy(thiz->name, XcpClientHandler_Name, CHANNEL_HANDLER_NAME_LEN);

        thiz->invalid = false;
        thiz->onRemove = XcpClientHandler_Delete;
        ...
        thiz->context = XcpClientHandlerContext_New(device);
        if (thiz->context == NULL)
        {
            LOG_E(TAG, "XcpClientHandlerContext_New FAILED");
            ret = TINY_RET_E_NEW;
            break;
        }
    } while (0);
...
ChannelHandler * XcpClientHandler(Device *device)
{
    ...
    do
    {
        ...
        if (RET_FAILED(XcpClientHandler_Construct(thiz, device)))
        {
            XcpClientHandler_Delete(thiz);
            thiz = NULL;
            break;
        }
    } while (0);
